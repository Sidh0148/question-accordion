...
// Set up keyboard navigation and interactivity
accordionButtons.forEach((button, index) => {
  ...
  button.addEventListener("keydown", function (event) {
    if (event.key === "Enter" || event.key === " ") {
      ...
      const nextButton =
        accordionButtons[nextIndex < 0 ? accordionButtons.length - 1 : nextIndex % accordionButtons.length];
      nextButton.focus(); // Move focus to the next/previous button
    }
  });
});
